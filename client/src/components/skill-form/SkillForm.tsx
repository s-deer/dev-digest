  ...
  return (
    <div style={s.form}>
      ...
      <FormField label={t("form.description")} hint={t("form.descriptionHint")} required>
        <TextInput value={draft.description} onChange={set("description")} placeholder={t("form.descriptionPlaceholder")} aria-label={t("form.description")} />
      </FormField>
      <FormField label={t("form.type")}>
        <SelectInput value={draft.type} onChange={(value) => set("type")(value as SkillType)} options={SKILL_TYPES.map((type) => ({ value: type, label: t(`type.${type}`) }))} />
      </FormField>
      <FormField
        label={t("form.body")}
        ...
        right={
          <div style={s.bodyTabs}>
            <Button size="sm" kind={previewing ? "ghost" : "secondary"} onClick={() => setPreviewing(false)}>{t("form.write")}</Button>
            <Button size="sm" kind={previewing ? "secondary" : "ghost"} onClick={() => setPreviewing(true)}>{t("form.preview")}</Button>
          </div>
        }
      >
        {previewing ? (
